...
sub import
{
    ...
    %cornerz = map { ( uc $order[$_], $trig[$_] ) } ( 0..3 );
...

{
    ...
    sub generate
    {
		...
		defined( my $offset = shift )
			or croak "Bogus generate: missing offset";

		$offset =~ /^[01]/
			or croak "Bogus generate: $offset not 0 or 1";

		my $cache = shift
			or croak "Bogus generate: missing w-curve cache";
		...
		for( @$cache )
		{
			...
			{
				# filler for alt sequences shorter than
				# the base sequence.
...

{
	...
	{
		my $wcache = shift
			or croak "Bogus compare_w_curve: missing wcache";
		...
		for( @$wcache )
		{
			...
			{
				# if only one leg is defined then the
				# difference is equal to that leg.
